...
exports.seek = (function(){
      function seek_it(urla,classify,page,orderfield,callback){
              ...
              var classifyArr = area_class.classify.get_fullname_by_id(classify);
               /*
               *老天，不是我想把orderfield打成orderfiled的
                */
              var url = baseUrl + urla +"?pno="+page+ "&orderfiled="+orderfield+"&sceneServe=-1&serverType=-1&provideBill=-1&classes=" + encodeURIComponent(classifyArr);
    ...
    function games(page,orderfield,callback) {
        var urla = "/search/game.action";
        if(!orderfield){
            orderfield = "";
        }
        try{
            var url = baseUrl + urla +"?pno="+page+ "&orderfiled=" +orderfield;
            request(url,function(error,response,body){
                if(error)
                    console.log(error);
                var resultObj = body;
