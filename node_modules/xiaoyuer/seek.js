...
exports.seek = (function(){
      function seek_it(urla,classify,page,callback){
          try{
              var classifyArr = area_class.classify.to_param(classify);

              var url = baseUrl + urla +"?currentPage="+page+ "&sceneServe=1&cashUint=0&provideBill=-1";
              classifyArr.forEach(function(value){
                  url += "&"+ value.index +"=" +value.value;
              })
    ...
    function seek_it_wel(urla,classify,page,callback){
        try{
            var url = baseUrl + urla +"?currentPage="+page+ "&sceneServe=1&cashUint=0&provideBill=-1";
            var classifyArr = area_class.welfare_calssify.to_param(classify);
            classifyArr.forEach(function(value){
                url += "&"+ value.index +"=" +value.value;
            })
    ...
    function wel_req(classify,page,callback) {
        ...
        try {
            seek_it_wel(url, classify, page, callback);
        }
        catch (e) {
            callback(JSON.stringify({code: "-1"}));
            console.log(e);
...
exports.collet = function(id,openid,type,callback){
    var url = baseUrl + "collect/"+type+".action?id=" + id + "&openid=" + openid+"&type="+type;
    var obj = {
        id:id,
        openid:openid
    }
