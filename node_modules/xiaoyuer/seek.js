...
exports.seek = (function(){
      function seek_it(urla,classify,page,callback){
          try{
              var classifyArr = [];
              var arr = classify.split("-");
              var temp ={};
              for (var i = arr.length;i > 0; i -= 1){
                  temp.index = "classify" + i;
                   temp.value = arr[i-1];
                  classifyArr[classifyArr.length] = temp;
              }
    ...
    function seek_it_wel(urla,classify,page,callback){
        try{
            ...
            var url = baseUrl + urla +"?currentPage="+page+ "&sceneServe=1&cashUint=0&provideBill=-1" +"&classes=" +classify;
