...
function award_info(){
    ...
    *@description 中奖之后的处理。即减少奖品的剩余个数
    ...
    this.after_luck = function(id,award_level){
        var sql = "UPDATE award_" + id + " SET award_left=award_left-1 WHERE award_level = " + award_level;
        (function minus_it(){
            db.query(sql,function(err,result){
                 if(err){
                     console.log(err);
...
function lottery_result(){
    ...
    *增加一个抽奖结果
    ...
    this.add_result = function(obj){
        ...
        obj.is_recieved =  obj.is_award?0:1;
...
function lottery_info(){
    ...
    this.add_one = function(obj,callback){
        ...
        var sql = sprintf.sprintf(sql_temp,obj.award_level,obj.award_id.id,obj.address_info,obj.mobile_number);
        db.query(sql,function(err,result){
            if(err)
                console.log(err);
...
*获取获奖后的奖品信息
 */
exports.getAwardAddInfo = function(the_code,callback){
    lotteryInfo.get_by_id(the_code,function(result){
       callback(result);
    })
}
           ...
           awardInfo.get_all_award(result.id,function(result1){
               ...
               *设定每个中奖的区间
               ...
               result1.forEach(function(value){
                    if(value.award_left){
                        ...
                        award_pool[award_pool.length-1].min = now_position;
                        award_pool[award_pool.length-1].max = now_position + award_pool[award_pool.length-1].award_prob;
                        now_position = award_pool[award_pool.length-1].max;
                    }
               })
              ...
              var award_result = find_it(Math.random());
               if(award_result == -1){
                   ...
                   *移除一个奖品
                    */
                   awardInfo.after_luck(result.id,award_pool[award_result].award_level);
                   var obj_info = {
