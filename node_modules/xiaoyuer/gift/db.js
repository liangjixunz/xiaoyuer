...
*判断是否为招聘
 */
exports.is_hire = function(event_id,callback){
    var sql;
    sql  = "SELECT email FROM event_index WHERE event_id = '" + event_id +"'";
    try{
        db.query(sql,function(err,result){
            if(err)
                throw err;
...
exports.new_withdraw = function(openid,cash,mobile,callback){

    get_balance(openid,function(result){
        if(result - cash >= 0){
            var id = new Date().getTime();
            var sql_temp;
            sql_temp = "INSERT INTO event_withdraw VALUES('%s','%s','%s','%s',%d,'%s')" ;
            var sql = sprintf.sprintf(sql_temp,id,openid,cash,new Date(id).Format("yy-MM-dd hh:mm:ss"),0,mobile);
            db.query(sql,function(err,result1){
                if(err)
                    throw err;
            })
            set_balance(openid,cash,function(result1){
                callback(id);
            })
...
function get_balance(openid,callback){
    var sql_temp =  "SELECT * FROM user_info WHERE openid = '%s'";
    var sql = sprintf(sql_temp,openid);
    try{
        db.query(sql,function(err,result){
            if(err) throw err;
            callback(result[0].user_balance);
            console.log(result[0].user_balance)
        })
    }
    catch(e){
        callback(-1);
...
function set_balance(openid,cash,callback){
    var sql_temp =  "UPDATE user_info SET user_balance=user_balance-%s ,withdraw=withdraw+%s WHERE openid = '%s'";
    var sql = sprintf(sql_temp,cash,cash,openid);
    try{
        db.query(sql,function(err,result){
            if(err) throw err;
            callback(result);
...
exports.add_event =function(info_obj,callback){
    ...
    try{
        var sql = sprintf(sql_temp,id,info_obj.title,info_obj.total_award,info_obj.total_award,info_obj.start_time,info_obj.finish_time,info_obj.max,(info_obj.email?1:""));
        db.query(sql,function(err,result){
            if(err)throw err;
        });
